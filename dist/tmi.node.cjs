...
// src/Client.ts
var import_irc_parser = require("@tmi.js/irc-parser");
...
function getUser(tags) {
  ...
  return {
    ...
    isVip: "vip" in tags && tags.vip === !0 || badges.has("vip"),
    type: tags.userType
  };
}
...
`).forEach((line) => this.onIrcLine(line));
  ...
  async onSocketOpen(event) {
    ...
    let token = "schmoopiie", isAnon = this.identity.isAnonymous();
    if (!isAnon) {
      let tokenValue = await this.identity.getToken();
      tokenValue ? token = `oauth:${tokenValue}` : isAnon = !0;
    }
    this.didConnectAnonymously = isAnon, this.sendIrc({ command: "CAP REQ", params: ["twitch.tv/commands", "twitch.tv/tags"] }), this.sendIrc({ command: "PASS", params: [token] }), this.sendIrc({ command: "NICK", params: [isAnon ? "justinfan123456" : "justinfan"] });
  }
  onSocketError(event) {
  ...
  onIrcLine(line) {
    let ircMessage = (0, import_irc_parser.parse)(line, parseTag);
  ...
  handleUSERSTATE({ tags, channel: channelName }) {
    let channel = this.getChannelByLogin(channelName) ?? this.getChannelPlaceholder(void 0, channelName), { badges } = tags, user = {
      id: this.identity.id,
      color: tags.color,
  ...
  sendIrc(opts) {
    let result = (0, import_irc_parser.format)({
      ...opts,
